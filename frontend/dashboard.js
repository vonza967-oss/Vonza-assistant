...
const DASHBOARD_HU_PHRASES = Object.freeze({
  ...
  "Browser voice/Web Call setup": "Böngészős hang/Web Call beállítása",
  ...
  "Turn-based browser voice for the hosted Front Desk page. It requires voice input and spoken replies, and uses AI capacity and rate limits.": "Fordulóalapú böngészős hang a hosztolt Front Desk oldalhoz. Hangbevitel és felolvasott válaszok szükségesek hozzá, AI-kapacitást és sebességkorlátokat használ.",
  "Incomplete": "Hiányos",
  ...
  "Enable Web Call last.": "Végül engedélyezd a Web Callt.",
    ...
    <section class="dashboard-loading-screen" role="status" aria-live="polite" aria-busy="true" aria-label="${escapeHtml(t("app.loading.title"))}">
      <div class="dashboard-loading-shell">
        ...
        <p class="dashboard-loading-reassurance">
          <span>${escapeHtml(t("app.loading.footer"))}</span>
          <span class="dashboard-loading-delayed">${escapeHtml(t("app.loading.delayed"))}</span>
        </p>
        <div class="dashboard-loading-actions" aria-label="Loading actions">
          <button class="ghost-button" type="button" data-loading-refresh>${escapeHtml(t("app.loading.refresh"))}</button>
        </div>
      </div>
    </section>
  `;
  document.querySelector("[data-loading-refresh]")?.addEventListener("click", () => {
    window.location.reload();
  });
}
...
async function fetchJson(url, options) {
  ...
  if (!response.ok) {
    throw new Error(getDashboardApiErrorMessage(data));
  }
...
async function refreshAgentInstallState(agentId, options = {}) {
  ...
  try {
    ...
    try {
      await loadActivationWizard(agentId);
    } catch (error) {
...
async function boot(options = {}) {
  ...
  if (DASHBOARD_LOCAL_FIXTURE_ENABLED) {
    const fixtureRenderer = window.VonzaDashboardFixture?.renderLocalDashboardV2Fixture;

    if (typeof fixtureRenderer !== "function") {
      setStatus("Local dashboard fixture is unavailable.");
      renderErrorState(
        "Local dashboard fixture unavailable",
        "The local-only fixture script did not load, so the dashboard preview cannot render."
      );
      return;
    }
  ...
  try {
    ...
    if (showBootLoading) {
      renderLoadingState();
      setStatus(authCopy("Opening your workspace...", "Megnyitjuk a munkaterületedet..."));
    } else {
      setDashboardBackgroundRefreshing(true, options.activeAction || "workspace-refresh");
    }
