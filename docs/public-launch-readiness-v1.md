...
- Sell Vonza as a Hungarian-first Website Widget SaaS for SMEs: a website AI agent/widget for questions, quote intent, booking intent, contact capture, and follow-up on existing business websites.
- Public launch promise: install a Hungarian website AI agent/widget in about 5 minutes, with no technical skill required.
- Stable launch core: Website Widget setup, website import, template/tone setup, install verification, allowed domains, Home, Customers, Front Desk, Analytics, Customize, lead capture, and the shared owner dashboard.
- The AI Front Desk is the broader system behind the widget. Keep the full-page Front Desk as a companion/expansion channel for QR/direct links, WordPress pages, smart embeds, and dedicated customer-facing flows.
...
- Verify the core paid-user path: homepage -> pricing -> signup/checkout -> auth -> dashboard -> install Website Widget.
- Verify website URL save and website import work on a real customer-style site.
- Verify template/tone setup, one-line Website Widget install, install detection, and allowed-domain checks on a real published page.
- Verify the first live Website Widget conversation uses Hungarian-first defaults and grounded website answers.
