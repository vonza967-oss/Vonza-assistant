...
Connected Apps Phase 9 adds `src/services/integrations/googleConnectedAppAdapter.js` for Google Calendar only. The adapter mirrors successful existing Google Calendar connections and token-refresh issue states into `connected_app_connections` with redacted provider account label, capability keys, granted scope metadata, and status. Google Calendar Phase 1 hardens that lifecycle only: stale, expired, refresh-failed, and permission-missing Google account states map to `needs_attention`; `disabled` and `revoked` remain distinct; reconnect updates the existing owner/agent Google account and mirror; local disconnect marks the account/mirror revoked, clears encrypted token material, and disables matching enablements; OAuth failures store safe reason codes and reconnect guidance. It does not add new Google scopes, does not create a new OAuth flow, does not create agent enablements automatically, does not add public chat/tool execution, does not add widget/embed behavior, does not enforce package activation, and does not expose tokens or OAuth codes. Provider-side Google revoke remains future.
