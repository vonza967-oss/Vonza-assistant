...
Connected Apps Phase 5 implements the minimal generic persistence and internal service foundation from the Phase 4 design. It adds canonical owner connection and agent enablement tables plus a service skeleton. Connected Apps Phase 6 adds a report-only readiness context helper that can derive `evaluateConnectedAppReadiness()` input from those generic records. Connected Apps Phase 7 exposes those records through authenticated owner-scoped dashboard API routes only. Connected Apps Phase 8 adds an authenticated dashboard/settings management surface over those routes. Connected Apps Phase 9 adds Google Calendar as the first provider-specific adapter into the generic records by mirroring the existing Google operator connection flow. Connected Apps Phase 10 adds a WhatsApp Business capability foundation: registry metadata, safe manual/status-only connection representation, readiness tests, dashboard copy, and docs. Connected Apps Phase 11 adds a scoped WhatsApp webhook verification/readiness route and service only. Connected Apps Phase 12 adds WhatsApp POST signature helper support, raw-body route wiring, and redacted inbound webhook event normalization only. Connected Apps Phase 13 adds redacted inbound event storage for audit and future routing preparation only. Connected Apps Phase 14 adds manual read-only inbound staff inbox thread grouping and owner/dashboard review APIs only. The manual staff reply milestone adds `connected_app_outbound_messages`, a feature-flagged owner route, service-only WhatsApp Cloud API credential/destination lookup, and a dashboard manual composer for staff-authored replies only. These phases do not add runtime chat behavior, widget/embed behavior, new Google scopes, generic OAuth setup, WhatsApp OAuth/Embedded Signup, WhatsApp inbound chat handoff, AI replies, automatic WhatsApp replies, package activation enforcement, external API calls beyond existing Google operator workflows and the feature-flagged manual WhatsApp staff route, generic provider clients, generic provider execution, or secrets.

Google Calendar Phase 1 now hardens the existing Phase 9 adapter lifecycle only: status normalization, reconnect recovery, local disconnect/revoke, and sanitized OAuth failure reporting. It adds no new Google scopes, no public chat calendar execution, no widget/embed behavior, no package activation enforcement, and no broad Calendar feature expansion. Provider-side Google revoke is still future because Phase 1 uses the existing local token-storage pattern and does not introduce an untested Google revoke client path.
...
## Implemented Inbound Event Audit: `connected_app_inbound_events`

Connected Apps Phase 13 adds `connected_app_inbound_events` as a provider-neutral storage foundation for redacted inbound webhook event summaries. It is an audit/routing-preparation table only. It is not a staff inbox, customer inbox, chat handoff queue, AI reply trigger, outbound send queue, package activation gate, or runtime permission grant.

Implemented fields include owner and connection scope, optional agent scope, provider/app/capability keys, provider event/message ids, provider timestamp, source account/channel ids, inbound direction, received/ignored/duplicate/invalid status, redacted normalized JSON, redaction summary JSON, dedupe key, metadata, received timestamp, and created timestamp.

The Phase 13 service accepts only normalized/redacted event summaries. It rejects raw provider payload fields, message body text, contact phone/profile fields, token/secret-looking values, URL-looking metadata, and full payload/body/contact arrays. For WhatsApp, POST stores one safe row per normalized message/status/unknown summary and dedupes by owner/provider/message-derived dedupe key when possible. It does not create `messages`, `agent_contact_leads`, `agent_action_requests`, `agent_booking_requests`, operator contacts, booking requests, outbound replies, or external provider calls.

## Implemented Manual Inbound Threads: `connected_app_inbound_threads`

Connected Apps Phase 14 adds `connected_app_inbound_threads` as the provider-neutral grouping foundation for a manual read-only staff inbox. Threads group redacted inbound event rows by owner, connection, provider, app, capability, optional agent, and a hashed external conversation key. The external conversation key may be derived from a provider webhook payload, but the raw customer identifier is used only transiently for hashing and is never stored or returned.

Implemented fields include owner and connection scope, optional agent scope, provider/app/capability keys, `external_thread_key_hash`, safe `external_thread_label`, review `status` constrained to `open`, `reviewing`, `resolved`, `ignored`, `archived`, last event id/time/type, last message type, unread count, optional owner assignment, redacted metadata, and created/updated timestamps.

The thread service is owner-scoped, validates provider/app/capability against the registry, rejects unsafe metadata, stores no message bodies, profile names, full provider payloads, customer phone numbers, contacts, leads, action requests, booking requests, or chat messages, and never sends outbound provider messages. Unread count increments only for inbound message events. WhatsApp delivery/status events can update last-event fields but do not increment unread. Duplicate webhook retries reuse existing event/thread rows.

## Implemented Manual Outbound Audit: `connected_app_outbound_messages`

The manual reply milestone adds `connected_app_outbound_messages` as an owner-readable audit table for staff-initiated WhatsApp replies. Writes are service/internal only; RLS grants authenticated owners `select` and grants no anon access and no authenticated insert/update/delete policies.

Implemented fields include owner, connection, optional agent, thread, provider/app/capability, `destination_ref_hash`, `message_type`, redacted body summary, optional template name/language, lifecycle `status` constrained to `draft`, `queued`, `sent`, `failed`, or `blocked`, manual approval mode, provider message/status ids when safe, redacted error code/message, metadata, manual actor id, sent timestamp, and created/updated timestamps.

The table intentionally stores no destination phone number, access token, app secret, verify token, webhook secret, raw provider payload, or full provider error payload. `body_redacted` stores only a staff-text redaction marker with length; the staff-authored text is not persisted in audit rows. Metadata states that the row is manual staff reply audit, message text was not stored, provider payload was not stored, no AI reply was used, and no automatic WhatsApp message was sent.

`src/services/integrations/whatsappManualReplyService.js` is provider-specific, not a generic connected-app execution engine. It requires `WHATSAPP_MANUAL_REPLIES_ENABLED` to be explicitly enabled with `1`, `true`, `enabled`, or `on`, requires a manual actor id, validates owner/thread/connection scope, validates active WhatsApp connection and capability, validates optional agent enablement, blocks templates until approved-template support exists, enforces a conservative internal customer-service-window guard from inbound thread metadata, resolves destination and Cloud API credentials only through server-side dependencies/configuration, and calls an injectable provider client only after all checks pass.
...
- `GET /agents/connected-app-inbound-threads`: lists the authenticated owner's redacted inbound threads with optional `provider`, `connectionId`, `agentId`, `status`, `threadId`, and `limit` filters.
- `POST /agents/connected-app-inbound-threads/status`: updates only the authenticated owner's thread review status. It does not accept reply text, send controls, AI draft controls, handoff controls, tokens, or provider execution fields.
- `POST /agents/connected-app-inbound-threads/reply`: sends a staff-authored WhatsApp manual reply only when the feature flag and manual reply service checks pass. It rejects unauthenticated requests, token/secret fields, phone/destination fields, provider payloads, connection/provider overrides, nested objects, AI draft controls, and automatic-send controls. It returns only a safe outbound DTO.
- `GET /agents/connected-app-inbound-events`: lists the authenticated owner's redacted inbound event rows with optional `provider`, `connectionId`, `agentId`, `status`, `threadId`, and `limit` filters.

All connected-app owner routes are authenticated owner/internal setup or manual-review APIs only. There are no public or anonymous connected-app routes. Except for the feature-flagged manual WhatsApp staff reply route, the routes do not call external providers. The manual route is not generic provider execution, not AI, not automatic, not public chat/tool use, not package activation enforcement, not OAuth/Embedded Signup, not Twilio WhatsApp, and not widget/embed UI.

## Implemented Authenticated Dashboard Surface

Phase 8 adds a compact `Connected apps` management surface in the authenticated dashboard/settings area. Phase 9 updates that surface to show Google Calendar as a real adapter backed by the existing Google connection flow. Phase 10 adds a WhatsApp Business foundation panel for manual/status-only readiness. Phase 11 does not add credential inputs or sender controls; the existing status surface can show webhook status from generic records. It fetches:
...
It shows provider and capability labels, owner connection status, provider account labels, scopes/capability summaries, webhook status, agent enablement state, approval mode, allowed surfaces, report-only readiness warnings, redacted inbound thread status, safe redacted thread labels, unread counts, last event type/message type, and recent redacted events. It can reuse the existing Google connect button for Google Calendar, create a manual/status-only connection record for non-adapter review, update a connection status, create/update an agent enablement, refresh the read-only inbox, and mark threads `reviewing`, `resolved`, `ignored`, or `archived`.

The Phase 9 dashboard copy is explicit: `Uses existing Google connection flow`, `No chat execution`, `No provider action without approval`, and `Report-only readiness`. The WhatsApp inbox copy is explicit: `Manual staff reply`, `No AI reply`, and `No automatic WhatsApp messages`. Manual/status-only records remain available for non-adapter review. The Google Calendar adapter, WhatsApp Business foundation panel, and Connected app inbox do not show or accept raw tokens, secrets, OAuth URLs, webhook URLs, provider client fields, executable handler fields, public chat callable controls, package selector/package switching controls, WhatsApp token inputs, WhatsApp app-secret inputs, phone number display, AI draft buttons, automatic-send controls, template-management UI, or destination/phone inputs. The WhatsApp manual composer appears only when the server feature status is enabled, accepts only staff-authored text, and posts to the owner-scoped manual reply route. The dashboard does not add a new OAuth flow, enforce package activation, expose public/anonymous connected-app management routes, change runtime chat behavior, or change widget/embed bundles.
...
- Google OAuth/calendar tables: Keep `google_oauth_states` and `google_connected_accounts` for the operator workspace. The Phase 9 adapter coexists with those tables and does not replace their owner/agent checks. Google Calendar Phase 1 reconnect updates the existing owner/agent account row when provider identity or email matches, removes `google.calendar.write` from the mirror when the granted write scope is downgraded, and local disconnect marks the Google account and mirror `revoked` while clearing encrypted token material. It does not delete historical calendar events or call a provider revoke endpoint.
...
- WhatsApp Business: Phase 10 does not add a provider-specific table or adapter. Phase 11 uses the existing generic connection row for webhook verification/readiness only. Phase 12 adds signature helpers and redacted inbound event normalization. Phase 13 stores those redacted summaries in `connected_app_inbound_events` for audit/routing preparation only. Phase 14 groups safe WhatsApp message/status events into `connected_app_inbound_threads` for manual read-only staff review only. The connection can represent safe manual metadata such as WhatsApp Business Account ID, phone number ID, business display name, webhook verification status, Graph API version, webhook verification timestamp, last webhook receipt timestamp, last webhook object, safe event type names, safe message type names, and signature status. The derived verify-token reference belongs in `token_secret_ref`, not public metadata, and `token_secret_ref` must not be overloaded with a raw Meta app secret. Access tokens, app secrets, raw verify tokens, webhook secrets, copied provider payloads, customer contact phone numbers, profile names, message bodies, and endpoint URLs must not live in metadata, event rows, thread rows, or route responses. Until a future service-only secret store or provider-specific signing-secret reference exists, POST signature status remains `not_configured` rather than verified. Future WhatsApp work must separately handle service-only app-secret storage/signing-secret references, full signature enforcement, consent/session windows, manual staff replies, AI-drafted replies after manual reply workflows, session replies, and approved template messages.
...
- Owner users may select only their own redacted inbound event and inbound thread rows.
...
- No public or anonymous connected-app management routes.
...
- No WhatsApp inbound-to-chat processing, chat handoff, AI replies, WhatsApp replies, outbound message sending, Meta OAuth/Embedded Signup, Cloud API calls, Twilio WhatsApp API calls, app-secret signature validation, template execution, session reply execution, or package activation enforcement.
- No secrets committed.

## Implementation Status

Phase 5 is implemented as schema, migration, service, tests, and docs only. Phase 6 is implemented as a service/tests/docs readiness adapter over the generic records only. Phase 7 is implemented as authenticated owner-scoped routes, service tightening, tests, and docs only. Phase 8 is implemented as authenticated dashboard UI/tests/docs only. Phase 9 is implemented as a Google Calendar adapter over the existing Google operator connection flow. Phase 10 is implemented as a WhatsApp Business capability foundation with metadata/readiness/manual-status/dashboard-copy/docs/tests only. Phase 11 is implemented as WhatsApp webhook verification/readiness only. Phase 12 is implemented as WhatsApp signature helpers and redacted normalization only. Phase 13 is implemented as redacted inbound event storage only. Phase 14 is implemented as manual read-only inbound staff inbox thread grouping only. Generic persistence now exists, readiness can be derived from those records, authenticated owners/internal setup paths can create/list/update redacted generic connection and enablement records, successful Google Calendar connections mirror into `connected_app_connections`, WhatsApp Business can be represented manually with safe non-secret metadata, the scoped WhatsApp webhook route can verify setup, record safe readiness metadata, persist redacted inbound event audit rows, and group safe WhatsApp message/status events into redacted owner-scoped threads. The dashboard can reuse the existing Google connect flow, show WhatsApp Business foundation status, manage manual/status-only records, explicitly enable selected capabilities for an agent, show report-only readiness, and show a manual read-only Connected app inbox. No new Google scopes, generic OAuth/provider setup, Meta OAuth/Embedded Signup, WhatsApp chat handoff, WhatsApp AI replies, WhatsApp replies, WhatsApp message sender, app-secret signature validation, runtime permission enforcement, generic external API/provider execution, widget/embed behavior, runtime chat behavior, package activation enforcement, package switching, or generic secret storage exists yet. The current connected app registry and readiness services remain metadata/report-only, and provider-specific integrations remain provider-specific unless a scoped adapter explicitly mirrors redacted state into generic records.
