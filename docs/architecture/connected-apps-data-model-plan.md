...
- WhatsApp Business: Phase 10 does not add a provider-specific table or adapter. Phase 11 uses the existing generic connection row for webhook verification/readiness only. The connection can represent safe manual metadata such as WhatsApp Business Account ID, phone number ID, display phone number, business display name, webhook verification status, Graph API version, webhook verification timestamp, last webhook receipt timestamp, last webhook object, and safe event type names. The derived verify-token reference belongs in `token_secret_ref`, not public metadata. Access tokens, app secrets, raw verify tokens, webhook secrets, copied provider payloads, contact phone numbers, message bodies, and endpoint URLs must not live in metadata or route responses. Future WhatsApp work must separately handle app-secret signature validation, inbound message normalization, consent/session windows, session replies, and approved template messages.
