...
- WhatsApp Business: Phase 10 does not add a provider-specific table or adapter. Phase 11 uses the existing generic connection row for webhook verification/readiness only. Phase 12 adds signature helpers and redacted inbound event normalization. Phase 13 stores those redacted summaries in `connected_app_inbound_events` for audit/routing preparation only. The connection can represent safe manual metadata such as WhatsApp Business Account ID, phone number ID, display phone number, business display name, webhook verification status, Graph API version, webhook verification timestamp, last webhook receipt timestamp, last webhook object, safe event type names, safe message type names, and signature status. The derived verify-token reference belongs in `token_secret_ref`, not public metadata, and `token_secret_ref` must not be overloaded with a raw Meta app secret. Access tokens, app secrets, raw verify tokens, webhook secrets, copied provider payloads, customer contact phone numbers, profile names, message bodies, and endpoint URLs must not live in metadata, event rows, or route responses. Until a future service-only secret store or provider-specific signing-secret reference exists, POST signature status remains `not_configured` rather than verified. Future WhatsApp work must separately handle service-only app-secret storage/signing-secret references, full signature enforcement, routing, consent/session windows, staff inbox handoff, session replies, and approved template messages.
