...
The Phase 9 dashboard copy is explicit: `Uses existing Google connection flow`, `No chat execution`, `No provider action without approval`, and `Report-only readiness`. Phase 14 WhatsApp inbox copy is explicit: `Inbound review only`, `No WhatsApp replies sent`, `No AI handoff`, and `No outbound messaging`. Manual/status-only records remain available for non-adapter review, but the Google Calendar adapter, WhatsApp Business foundation panel, and Connected app inbox do not show or accept raw tokens, secrets, OAuth URLs, webhook URLs, provider client fields, executable handler fields, public chat callable controls, package selector/package switching controls, WhatsApp token inputs, WhatsApp app-secret inputs, phone number display, reply boxes, send buttons, AI draft buttons, or message-send controls. They do not add a new OAuth flow, call providers from the generic surface, enforce package activation, expose public/anonymous connected-app management routes, change runtime chat behavior, or change widget/embed bundles.
