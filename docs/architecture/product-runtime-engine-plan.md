...
- Enterprise Request Desk is now a separate full-page product shell with public product/setup/intake/dashboard routes and owner-scoped setup plus request persistence. It is still not imported into the runtime package registry, not dashboard-selectable from the generic package selector, and not wired to QDH, widget/embed, provider execution, operations cockpit, or quote pricing behavior.
