...
- Connected Apps Phase 10 adds WhatsApp Business as a capability foundation only. It declares `whatsapp.business.webhook`, `whatsapp.business.send.template`, and `whatsapp.business.send.session.reply`; supports safe manual/status-only connection metadata and readiness reporting; and later dashboard copy now makes the WhatsApp inbox boundary explicit with `Inbound review only`, `No WhatsApp replies sent`, `No AI handoff`, `No outbound messaging`, and `No Meta OAuth/Embedded Signup yet`. It adds no runtime chat behavior, widget/embed exposure, Meta OAuth/Embedded Signup, WhatsApp Cloud API calls, Twilio WhatsApp API calls, outbound messages, package activation enforcement, generic provider execution, or secrets.
...
- Show dashboard copy for `Manual/internal setup`, `Inbound review only`, `No WhatsApp replies sent`, `No AI handoff`, `No outbound messaging`, and `No Meta OAuth/Embedded Signup yet`.
