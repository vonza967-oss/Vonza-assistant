...
- Allow only safe non-secret manual metadata such as WhatsApp Business Account ID, phone number ID, display phone number, business display name, webhook verification status, and Graph API version.
...
- Show dashboard copy for `Manual/internal setup`, `No WhatsApp messages sent`, `No webhook receiver enabled yet`, and `No Meta OAuth/Embedded Signup yet`.
