...
import {
  ...
  listHotelConciergeEvalScenarios,
...
function parseArgs(argv = []) {
  const options = {
    mode: process.env.HOTEL_CONCIERGE_EVAL_MODE || "dry-run",
    scenarioIds: [],
    limit: 0,
...
if (options.list) {
  listHotelConciergeEvalScenarios().forEach((scenario) => {
    console.log(`${scenario.id} [${scenario.categories.join(", ")}] ${scenario.title}`);
  });
  process.exit(0);
}

const report = await runHotelConciergeEvaluation(options);

if (options.json) {
  console.log(JSON.stringify(report, null, 2));
} else {
  console.log(formatHotelConciergeEvalReport(report));
