...
- `manifest.example.js`: manifest shape, including role, intents, prompt blocks, risk rules, tool declarations, future connected app requirements metadata, and knowledge policy.
...
- Keep `connectedAppRequirements.requiredCapabilities` and `connectedAppRequirements.optionalCapabilities` empty unless a later readiness-only PR explicitly scopes package requirements. This metadata is not provider setup, activation enforcement, or execution permission.
