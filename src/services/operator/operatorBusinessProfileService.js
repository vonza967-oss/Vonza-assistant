  ...
  if (error) {
    if (isMissingRelationError(error, OPERATOR_BUSINESS_PROFILE_TABLE)) {
      const unavailable = new Error(
        "Business profile persistence is not ready on the server yet. Apply the operator business profile migration and try again."
      );
      unavailable.statusCode = 503;
      unavailable.code = "operator_business_profile_persistence_unavailable";
      throw unavailable;
    }
