...
function createWidgetHarness({
  ...
  [
    ...
    "call-front-desk-mute",
    "call-front-desk-end",
    "call-front-desk-type",
...
test("voice input getUserMedia denial shows a user-facing permission error", async () => {
  ...
  assert.equal(harness.elements.get("composer-status").textContent, "Microphone access is blocked. Allow microphone access in your browser settings or type your message.");
  assert.equal(micButton.title, "Microphone access is blocked. Allow microphone access in your browser settings or type your message.");
});
...
test("voice input reports HTTPS requirement before requesting the microphone", async () => {
  ...
  assert.equal(harness.elements.get("composer-status").textContent, "Voice input requires HTTPS. Open this Front Desk over a secure link or type your message.");
});
...
test("call Front Desk turn transcribes, sends chat, and speaks with returned token", async () => {
  ...
  assert.equal(harness.elements.get("call-front-desk-mute").hidden, false);
  assert.equal(harness.elements.get("call-front-desk-mute").textContent, "Mute");
  ...
  assert.equal(harness.elements.get("call-front-desk-type").textContent, "Type instead");
...
test("call Front Desk mute and unmute control recording without sending a turn", async () => {
  ...
  const harness = createWidgetHarness({
    ...
    customFetch: async (input) => {
      ...
      if (url.includes("/api/voice/transcribe") || url === "/chat") {
        throw new Error("muted web call should not send voice or chat traffic");
      }
  ...
  assert.equal(harness.hooks.getCallModeState(), "listening");
  harness.elements.get("call-front-desk-mute").dispatch("click");
  await new Promise((resolve) => setTimeout(resolve, 0));

  assert.equal(harness.hooks.getCallModeState(), "waiting");
  assert.equal(harness.elements.get("call-front-desk-status").textContent, "Muted");
  assert.equal(harness.elements.get("call-front-desk-start").textContent, "Talk now");
  assert.equal(harness.elements.get("call-front-desk-mute").textContent, "Unmute");
  assert.equal(harness.elements.get("call-front-desk-type").hidden, false);
  assert.equal(calls.some((url) => url.includes("/api/voice/transcribe") || url === "/chat"), false);

  harness.elements.get("call-front-desk-mute").dispatch("click");
  await Promise.resolve();
  await new Promise((resolve) => setTimeout(resolve, 0));

  assert.equal(TestMediaRecorder.instances.length, 2);
  assert.equal(harness.hooks.getCallModeState(), "listening");
  assert.equal(harness.elements.get("call-front-desk-mute").textContent, "Mute");
});
...
test("dashboard install iframes separate section embed and full-page iframe while QR stays hosted", () => {
  ...
  const dashboardInstall = readFileSync(path.join(repoRoot, "frontend", "dashboardInstall.js"), "utf8");
  const widget = readFileSync(path.join(repoRoot, "frontend", "widget.html"), "utf8");
  const styles = readFileSync(path.join(repoRoot, "frontend", "style.css"), "utf8");
  const script = readFileSync(path.join(repoRoot, "frontend", "script.js"), "utf8");

  assert.match(dashboardInstall, /function buildFullPageAssistantUrl/);
  assert.match(dashboardInstall, /function getPublicPageKey/);
  assert.match(dashboardInstall, /url\.searchParams\.set\("k", publicPageKey\)/);
  assert.match(dashboardInstall, /function buildEmbeddedFullPageAssistantUrl/);
  assert.match(dashboardInstall, /function buildSmartAssistantEmbed/);
  assert.match(dashboardInstall, /url\.searchParams\.set\("embedded", "1"\)/);
  assert.match(dashboardInstall, /url\.searchParams\.set\("size", normalizedSize\)/);
  assert.match(dashboardInstall, /function buildSectionAssistantIframe/);
  assert.match(dashboardInstall, /assistant-embed\.js/);
  assert.match(dashboardInstall, /data-vonza-assistant/);
  assert.match(dashboardInstall, /data-background-scope="\$\{backgroundScope\}"/);
  assert.match(dashboardInstall, /data-layout="\$\{normalizedLayout\}"/);
  assert.match(dashboardInstall, /page-takeover/);
  assert.match(dashboardInstall, /normalizeFullPageBackgroundScope/);
  assert.match(dashboardInstall, /Recommended smart snippet/);
  assert.match(dashboardInstall, /Dedicated page embed snippet/);
  assert.match(dashboardInstall, /True page takeover snippet/);
  assert.match(dashboardInstall, /Advanced iframe snippet/);
  assert.match(dashboardInstall, /Raw iframe fallback/);
  assert.match(dashboardInstall, /Dedicated page embed makes the Front Desk the page body below your site header/);
  assert.match(dashboardInstall, /Use this on a blank assistant page\. It may hide the page footer and remove extra page spacing\./);
  assert.match(dashboardInstall, /Raw iframe backgrounds stay inside the iframe\. Use the smart dedicated page embed when you want the background to fill the page area\./);
  assert.match(dashboardInstall, /Recommended\. Automatically adjusts to most website layouts\./);
  assert.match(dashboardInstall, /Use this when Front Desk is the main content of a page on your website/);
  assert.match(dashboardInstall, /data-page-reset="true"/);
  assert.match(dashboardInstall, /data-hide-page-footer="true"/);
  ...
  assert.match(dashboardInstall, /data-show-title="false"/);
  assert.match(dashboardInstall, /show_title", "0"/);
  assert.match(dashboardInstall, /Advanced fallback\. Use this if your website builder does not allow scripts\./);
  assert.match(dashboardInstall, /layout: "canvas"/);
  assert.match(dashboardInstall, /buildEmbeddedFullPageAssistantUrl\(agent, "standard"\)/);
  assert.match(dashboardInstall, /buildEmbeddedFullPageAssistantUrl\(agent, "full", \{ surface: "flat", layout: "canvas", showTitle: options\.showTitle \}\)/);
  assert.match(dashboardInstall, /Place the Front Desk inside part of an existing page/);
  assert.match(dashboardInstall, /Use this when Front Desk is the main content of a page on your website/);
  assert.match(dashboardInstall, /\["compact", "standard", "tall", "full"\]/);
  assert.match(dashboardInstall, /surface: "flat"/);
  assert.match(dashboardInstall, /allow="microphone; autoplay"/);
  assert.match(dashboardInstall, /min-height:640px;border:0;border-radius:18px;overflow:hidden/);
  assert.match(dashboardInstall, /Dedicated page embed/);
  assert.match(dashboardInstall, /Copy smart snippet/);
  assert.match(dashboardInstall, /Copy raw iframe/);
  assert.match(dashboardInstall, /height:calc\(100vh - \$\{normalizedHeaderHeight\}px\);min-height:760px;border:0;display:block/);
  ...
  assert.match(dashboardInstall, /Customize Front Desk page/);
  assert.match(dashboardInstall, /data-settings-target="front_desk"/);
});
